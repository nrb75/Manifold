...
#It does not matter which ccame first, we simply want to know which pair occurs most frequently.
...
def format_rules(rules, orig_df,apps_server): 
    ...
    rules_df=rules_df.sort_values(by='latency_rank', ascending=False)#these must be sorted since we assign the important ones first
...
df_servers, server_rules80, total_latency, total_latency80, avg_latency, avg_latency80 = server_association(rules_df_80per_70con, df, 20)

#test outside of definition, see if it works
